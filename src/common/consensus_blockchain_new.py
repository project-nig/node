...
class ConcensusBlockChain:
    def __init__(self):
        from node.main import NEW_BLOCKCHAIN_DIR_BACKLOG,STORAGE_DIR
    ...
    def __setup__(self):
        ...
        try:
            #step 0 extraction of all the block vote
            #the blocks that are slashed are rejected
            filename='vote_book'
            import json
            from node.main import STORAGE_DIR,NEW_BLOCKCHAIN_DIR_BACKLOG
            vote_book_filename=STORAGE_DIR+NEW_BLOCKCHAIN_DIR_BACKLOG+f"/{filename}"
            vote_book_data={}
            try:
            ...
            for file in os.listdir(directory):
                ...
                try:vote_value=vote_book_data[block_PoH_Hash]
                except:vote_value=None
                if block_PoH_Hash!="last_block_pointer" and block_PoH_Hash!="vote_book" and block_PoH_Hash!=self.block_to_reject_now_by_leader_node and vote_value!="slash":
