...
'''
        ...
        #flag to check if an API call needs to be stored on the blockain
    ...
    def load_code_source(self):
        ...
        for i in range(100):
            value=load_smart_contract_from_master_state_leader_node(self.smart_contract_account,smart_contract_transaction_hash=smart_contract_transaction_hash)
            #logging.info(f"#CHECK value:{value}")
...
"""
    ...
    def load_gas(self):
        ...
        value=blockchain_base.get_smart_contract_api(self.smart_contract_account,smart_contract_transaction_hash=self.smart_contract_transaction_hash)
    ...
    def process(self):
        ...
        if self.type == "source":self.run_source(self.payload+process_memory)
        if self.type == "api":self.run_api(self.payload+process_memory)
        smart_contract_memory_full={}
    ...
    def run_smart_contract(self,expr):
        ...
        if self.gas_ok_flag is True:
            ...
            self.burn_gas()
            print(f"operation in {self.cpu} sec / memory {self.smart_contract_memory_size} / gas {int(self.gas)}")
...
def LOAD_SC(smart_contract_account,smart_contract_sender,payload):
    t="""
"""
...
def load_smart_contract_from_master_state(public_key_hash):
    ...
    if smart_contract_previous_transaction is not None:smart_contract_previous_transaction=smart_contract_previous_transaction[0:64]
    if smart_contract_transaction_hash is not None:smart_contract_transaction_hash=smart_contract_transaction_hash[0:64]
...
def load_smart_contract_from_master_state_leader_node(public_key_hash,*args, **kwargs):
    ...
    if smart_contract_transaction_hash is None:
        ...
        for transaction_hash in smart_contract_data["balance"]["credit"].keys():
            if transaction_hash[0:64]==smart_contract_transaction_hash[0:64]:smart_contract_content=smart_contract_data["balance"]["credit"][transaction_hash]
