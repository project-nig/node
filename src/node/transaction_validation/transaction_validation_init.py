...
from common.utils import normal_round,calculate_hash,check_marketplace_step1
...
class Transaction:
        ...
        for tx_input in self.inputs:
            ...
            if check_marketplace_step1(self.outputs) is False:
                try:
                    ...
                    try:
                        #logging.info(f"transaction_hash: {transaction_hash} output_index:{output_index}")
                        locking_script = self.blockchain.get_locking_script_from_utxo(unlocking_public_key_hash,transaction_hash, output_index)
    ...
    def get_total_amount_in_inputs(self) -> int:
        total_in = 0
        #Step 1 of Marketplace is not Checked as we're always using the same UTXO and total_amount_in_inputs=0
        if check_marketplace_step1(self.outputs) is False:
            for tx_input in self.inputs:
                unlocking_public_key_hash = tx_input["unlocking_public_key_hash"]
                if "SC " in unlocking_public_key_hash:
                    ...
                    unlocking_public_key_hash_list=unlocking_public_key_hash.split(" ")
                    unlocking_public_key_hash=unlocking_public_key_hash_list[2]
                transaction_data = self.blockchain.get_transaction_from_utxo(unlocking_public_key_hash,tx_input["transaction_hash"])
    ...
    def validate_funds(self):
        ...
        inputs_total = normal_round(self.get_total_amount_in_inputs(),ROUND_VALUE_DIGIT)
    ...
    def broadcast_to_leader_node(self):
        ...
        for node_dic in epoch['LeaderNodeList']:
            if node_dic['already_processed']==False:
                ...
                if node_dic['node'].hostname != self.hostname and node_dic['node'].hostname != self.sender:
            ...
            for i in range(len(self.outputs)):
                ...
                if index in smart_contract_index_list:
                    ...
                    try:
                        ...
                        if smart_contract_account=="31f2ac8088005412c7b031a6e342b17a65a48d01" and marketplace_transaction_flag is False and leader_node_flag is True and 5==6:
                            ...
                            old_smart_contract_previous_transaction,smart_contract_previous_transaction,smart_contract_transaction_output_index=load_smart_contract_from_master_state(smart_contract_account)
                            self.outputs[i]['smart_contract_previous_transaction']=smart_contract_previous_transaction
                            logging.info(f"###########Validating Smart Contract smart_contract_previous_transaction:{smart_contract_transaction_output_index}")
                            ...
                            input_list.append(TransactionInput(transaction_hash=smart_contract_previous_transaction,
                                                                output_index=smart_contract_transaction_output_index,
                                                                unlocking_public_key_hash=marketplace_owner.public_key_hash))

                            transaction_out_temp=TransactionOutput(public_key_hash=marketplace_owner.public_key_hash,
                            ...
                            new_transaction = NewTransaction(input_list, output_list)
                            new_transaction.sign(marketplace_owner)

                            self.inputs = [new_transaction.inputs[i].to_dict()]
                            self.outputs = [new_transaction.outputs[i].to_dict()]
                        ...
                        if leader_node_flag is True:self.outputs[i]['smart_contract_memory']=smart_contract.smart_contract_memory
                        ...
                        logging.info(f"**** ISSUE validate_smart_contract: {e}")
                        logging.exception(e)
                        self.is_smart_contract_valid = False
